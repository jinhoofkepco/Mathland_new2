...
-- Anonymous devices must not be able to project guardian identity columns.
revoke select on public.guardian_rewards from authenticated;
...
declare
  ...
  server_timestamp timestamptz := pg_catalog.statement_timestamp();
begin
  if pg_catalog.jsonb_typeof(event_payloads) <> 'array'
...
begin
  ...
  if publication_reason is null
    ...
    or pg_catalog.char_length(pg_catalog.btrim(publication_reason)) not between 1 and 500 then
    raise exception 'publication reason must contain between 1 and 500 characters'
      using errcode = '22023';
  end if;
  normalized_reason := pg_catalog.btrim(publication_reason);
...
comment on function public.commit_validated_content_publication(
  ...
  'Service-only atomic publish/rollback boundary. It requires a human reason, and normal publication must be numerically greater than every historical version; lower/equal versions are reachable only through validated rollback identity.';
