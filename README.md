...
운영 프로젝트에서는 이메일 가입과 Pages 콜백 URL을 허용한 뒤 migrations/functions를 배포합니다. 새 보호자는 매직 링크로 로그인해 가족 이름과 아이 별명만 입력하며, 서버의 단일 트랜잭션이 가족·보호자 권한·최초 아이·1회용 기기 연결 표식을 함께 만듭니다. 요청 재전송은 같은 가족을 반환하고, 이미 가입된 계정이나 익명 기기 계정의 가족 생성은 거부됩니다.
