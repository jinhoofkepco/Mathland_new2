...
stage_root="$test_root/project"
import_log="$test_root/import.log"
export_log="$test_root/export.log"
pck_path="$test_root/android-smoke.pck"

bash "$repo_root/scripts/android/stage_project.sh" "$repo_root" "$stage_root"
"$godot_bin" --headless --editor --path "$stage_root" --quit 2>&1 | tee "$import_log"
...
"$godot_bin" --headless --path "$stage_root" \
	--export-pack "Android Smoke" "$pck_path" 2>&1 | tee "$export_log"

node --input-type=module - \
	"$repo_root/assets/asset-manifest.json" \
	"$repo_root/content/active-manifest.json" \
	"$export_log" \
	"$pck_path" <<'NODE'
import assert from "node:assert/strict";
import { readFileSync, statSync } from "node:fs";

const [, , manifestPath, contentManifestPath, logPath, pckPath] = process.argv;
...
assert.ok(saved.has("content/active-manifest.json"), "active content manifest is missing from Android Smoke PCK");
for (const entry of contentManifest.packages) {
	assert.ok(
		saved.has(entry.path),
		`playable content package missing from Android Smoke PCK: ${entry.activity_id} (${entry.path})`,
	);
}

console.log(
  `Android Smoke PCK policy passed: ${manifest.assets.filter((asset) => asset.release).length} release assets, ${contentManifest.packages.length} content packages, ${statSync(pckPath).size} bytes`,
);
NODE
