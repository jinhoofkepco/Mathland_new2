...
export function verifyGeneratedAnswer(
  ...
  switch (generatorId) {
    ...
    case "multiplication_v1": {
      const operands = readIntegers(resolved.operands, issues, "operands");
      verifyInteger(answer, operands.length === 2 ? safeProduct(operands, issues) : null, issues);
      break;
    }
    ...
    case "prime_factorization_v1": {
      ...
      if (factors.some((factor) => !isPrimeByTrialDivision(factor))) issues.push("NON_PRIME_FACTOR");
      ...
      if (value !== null && safeProduct(factors, issues) !== value) issues.push("FACTOR_PRODUCT_MISMATCH");
      break;
    }
    case "counting_v1": {
      const count = readInteger(resolved.count, issues, "count");
      verifyInteger(answer, count, issues);
      if (readIntegers(resolved.item_ids, issues, "item_ids").length !== count) issues.push("ITEM_COUNT_MISMATCH");
      break;
    }
...
function verifyInteger(answer: Readonly<AnswerValueV1>, expected: number | null, issues: string[]): void {
