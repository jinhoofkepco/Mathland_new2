...
The nine Korean guide lines use one warm adult female synthetic guide voice. Every clip is skippable and replayable through a visible **Play / stop voice** control: `HomeVoiceButton` for the welcome, `SpeakerButton` for the six activity tutorials, and `RewardVoiceButton` on reward and level-up overlays. Activating the same control while its clip is playing stops it; activating it again replays it. Leaving an activity introduction or dismissing a reward/level-up overlay also stops the voice. Home/tutorial/reward/level-up clips may play only under their listed one-time or event policy and only when the caller explicitly authorizes autoplay; the default is silent. The in-activity control routes only the current activity's tutorial/instruction clip. It does not claim to narrate a dynamic mathematical expression, and voice playback never disables input or progression.
...
- SFX: each manifest row declares its reviewed measured full-clip RMS target with an exact ±0.1 dB tolerance, remains under 2 s except `level_up` and `reward` under 4 s, and has true peak at or below −0.5 dBFS. Integrated LUFS is intentionally not claimed for these very short effects;
